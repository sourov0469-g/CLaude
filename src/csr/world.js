...
const World = (function () {
  ...
  function buildScale() {
    ...
    /* the bolt it hangs on. Without this the card floats a quarter-unit off
       the column with nothing between the two, which is the one part of the
       scale that read as a mistake rather than an object: a brass backing
       plate against the shaft and two standoffs bridging the gap. */
    S.plaqueMount = new T.Group();
    S.plaqueMount.add(box(0.96, 0.56, 0.03, BRASS, {
      material: brassWarm, pos: [0, 2.20, 0.20],
    }));
    [-0.34, 0.34].forEach(function (x) {
      [-0.19, 0.19].forEach(function (y) {
        S.plaqueMount.add(cyl(0.028, 0.028, 0.20, 8, BRASS, {
          material: brassWarm, pos: [x, 2.20 + y, 0.11], rot: [Math.PI / 2, 0, 0],
        }));
      });
    });
    S.plaqueMount.visible = false;
    S.group.add(S.plaqueMount);
    ...
    /* the stem that carries the arc. The arc is centred on the pivot but sits
       0.30 behind it, so from the front it hung in the air off the back of the
       column; this drops a spine from the arc's midpoint onto the capital and
       braces it, which is what makes the gauge read as part of the instrument
       rather than a shape behind it. */
    S.group.add(box(0.07, 1.34, 0.07, BRASS, {
      material: brassWarm, pos: [0, 3.34, -0.30],
    }));
    [-1, 1].forEach(function (s) {
      S.group.add(box(0.045, 0.52, 0.045, BRASS, {
        material: brassWarm, pos: [s * 0.17, 3.02, -0.30], rot: [0, 0, s * 0.34],
      }));
    });
  ...
  function applyStage() {
    ...
    /* the backing plate and its standoffs arrive with the card, never before,
       so the post is bare until the framework slide puts something on it */
    S.plaqueMount.visible = S.plaque.visible;
    S.plaqueMount.scale.setScalar(Math.max(0.001, anim.plaqueOn));
